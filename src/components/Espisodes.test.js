...
test("renders show when data fetch finishes", () => {
  //initially episodes is an empty array when component mounts
  const { rerender } = render(<Episodes episodes={[]} />);

  //assertion: when the component first renders want the episodes array to be empty
  let episodesArr = screen.queryAllByTestId(/episodes/i);
  expect(episodesArr).toHaveLength(0);

    //rerendering component with episode data
    rerender(<Episodes episodes={episodeData} />);

  //assertions:
  //the data is on the screen after the rerender
